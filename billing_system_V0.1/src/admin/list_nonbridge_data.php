...
if($_POST['HdnDel'] == 'yes'){
    ...
    $DeleteQry = "DELETE FROM   tbl_bridge WHERE record_id = '$Delid'";
...
if (empty($_REQUEST['start_time']) === false) {
    ...
    if (empty($WhereQuery) === false)
        $WhereQuery .= " AND ";
    $WhereQuery.= " DATE(start_time) = '" . date('Y-m-d', strtotime($StartTime)) . "'";
}
if (empty($_REQUEST['end_time']) === false) {
    $EndTime = trim($_REQUEST['end_time']);
    if (empty($WhereQuery) === false)
        $WhereQuery .= " AND ";
    $WhereQuery .= " end_time = '" . date('Y-m-d', strtotime($EndTime)) . "'";
}
...
<!--                                            <th colspan="2" style="padding-left:5px;"  width="10%">Action</th>			-->
                        ...
                        $QryGetUsr = "select * from  tbl_bridge $WhereQuery";
