...
if(isset($_POST['status']) && $_POST['status']!=""){
	...
	$filter_cond=" pk_customer_id in (".implode(",",$unq_master).")";
}
				 ...
				 <input type="hidden" name="HdnMode" id="HdnMode" value="<?php echo $Page; ?>">
                  ...
                  <div class="" style="width:1100px;overflow:auto;">                      
                      <table class="sortable" align="center" cellpadding="2" cellspacing="2" valign="top"  border="0" > 
						...
						<?php
							$q=1;
							$Get_SelQry=mysql_query("select * from tbl_customer_filtered where $filter_cond");
							$intUsrCount=mysql_num_rows($Get_SelQry);
							 $TotalPages=ceil($intUsrCount/$RecordsPerPage);
                              $Start=($Page-1)*$RecordsPerPage;
							  $SelQry=mysql_query("select * from tbl_customer_filtered where id in (select id from tbl_customer_error) and $filter_cond limit $Start, $RecordsPerPage");
							if(mysql_num_rows($SelQry)>0){
								while($get_arr=mysql_fetch_assoc($SelQry)){
									...
									echo "<td></td><td align='center' rowspan='2'><a href='edit_glb_cust_err.php?errorid=".$get_arr['id']."&redto=yes' onclick='return funcalert()' title='Edit Customer Details'>Edit</a></td></tr><tr style='display:none' id='hidden_row_".$get_arr['pk_customer_id']."'><td></td><td  colspan='38'>
										<ul>
											<li id='cust_row_".$get_arr['pk_customer_id']."'></li>
										</ul>
									
									</td></tr><tr style='background:#f587a8' id='brg_row_err".$get_arr['pk_customer_id']."' onclick='funcajaxerrsh(\"".$get_arr['pk_customer_id']."\")'><td><img src='images/expand.png' width='30px' id='brg_row_img_err".$get_arr['pk_customer_id']."'></td>";


									$SelError_qry=mysql_query("select * from tbl_customer_error where id=".$get_arr["id"]);
									$error_arr=mysql_fetch_assoc($SelError_qry);
									foreach($error_arr as $e_key=>$err_cell){
										if($e_key!="id" && $e_key!="created_date" && $e_key!="modified_date" && $e_key!="pk_customer_id")
										echo "<td  align='center' nowrap>".$err_cell."</td>";
									}
									echo "</tr><tr style='display:none' id='hidden_row_err".$get_arr['pk_customer_id']."'><td></td><td  colspan='38'>
										<ul>
...
function funcajaxsh(custid){
	...
	$.ajax({
	  ...
	  data: { conferenceid: custid, rec_stat:status, redto:'yes'}
	})
	  .done(function( msg ) {
