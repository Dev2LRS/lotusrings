...
if(isset($_POST["redto"]))
	$redto="&redto=yes";
$sel_conf=mysql_query("select * from tbl_bridge_filtered where conference_pk_id='".$id."' $cond");
...
while($get_arr=mysql_fetch_assoc($sel_conf)){
	...
	$row.="<td align='center'><a href='edit_glb_brg_err.php?errorid=".$get_arr['id']."$redto' onclick='return funcalert()' title='Edit Bridge details'>Edit</a></td></tr>";
	$q++;
}
echo $head.$row."</table>";
}
