...
$chk_nb_qry=mysql_query("select * from tbl_bridge_raw where bridge_type='".$_POST["bridge_type"]."' and master_id='".$_POST["master_id"]."' and pk_bridge_id=".($u_id-1));
...
$data=array_values($final_brg);
	...
	$updt_brg=mysql_query("update tbl_bridge_error set $fdata where pk_bridge_id=$u_id");
if($updt_brg)
...
if(!empty($_GET["errorid"]) && preg_match('/^[0-9]+$/',$_GET["errorid"])){
	$id=$_GET["errorid"];
	$get_rec=mysql_query("select * from tbl_bridge_error where pk_bridge_id=$id");
