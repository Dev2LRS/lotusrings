...
if(isset($_POST["redto"]))
	$redto="&redto=yes";
$get_conf_cnt=mysql_query("select * from tbl_conference_raw where master_id='".$id."'");
$conf_cnt=mysql_num_rows($get_conf_cnt);
$sel_conf=mysql_query("select * from tbl_conference_error where master_id='".$id."'");
$cond='';
$finalot1='';
$finalot2='';
$head='<table style="font-size:12px;"><tr style="background-color:#f5fd99;">
							<th width="25px"></th>
...
while($get_arr=mysql_fetch_assoc($sel_conf)){
	...
	$row.="<td align='center'><a href='edit_glb_conf_err.php?errorid=".$get_arr['pk_conference_id']."$redto' onclick='return funcalert()' title='Edit Conference Details'>Edit</a></td></tr>
	...
	$q++;
	$cond.=" and pk_conference_id!='".$get_arr['pk_conference_id']."'";
}
$finalot1=$head.$row."</table>";
}

if($conf_cnt!=mysql_num_rows($sel_conf) || mysql_num_rows($sel_conf)==0)
	{	
		$body='';
		$sel_conf_sql=mysql_query("select * from tbl_conference_raw where master_id=$id".$cond);
		while($disp_arr=mysql_fetch_assoc($sel_conf_sql)){
			if($q%2==0)
				$rowclr='#f6f6f6';
			else $rowclr='#EEF2FD';
			 $body.="<tr style='background: $rowclr' id='brg_conf_".$disp_arr['pk_conference_id']."' onclick='funcajaxconf(\"".$disp_arr['pk_conference_id']."\")'><td><img src='images/expand.png' width='25px' id='brg_conf_img_".$disp_arr['pk_conference_id']."'></td>";
		...
		$finalot2=$head.$body."</table>";

	}
}
echo $finalot1.$finalot2;
//<a href='edit_glb_conf_err.php?errorid=".$disp_arr['pk_conference_id']."$redto' onclick='return funcalert()' title='Edit Conference Details'>Edit</a>
?>
