...
foreach($final_conf as $key=>$eachfield){
	...
	$updt_brg=mysql_query("update tbl_conference_error set $fdata where pk_conference_id=$u_id");
if($updt_brg)
...
if(!empty($_GET["errorid"]) && preg_match('/^[0-9]+$/',$_GET["errorid"])){
	$id=$_GET["errorid"];
	$get_rec=mysql_query("select * from tbl_conference_error where pk_conference_id=$id");
