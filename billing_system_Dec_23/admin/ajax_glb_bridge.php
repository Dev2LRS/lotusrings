...
if(isset($_POST["redto"]))
	$redto="&redto=yes";
$get_brg_cnt=mysql_query("select * from tbl_bridge_raw where conference_pk_id=$id");
$brg_cnt=mysql_num_rows($get_brg_cnt);
$sel_conf=mysql_query("select * from tbl_bridge_error where conference_pk_id='".$id."' $cond");
$head='<table style="font-size:11px;"><tr style="background-color:#dbd2ff;">			
...
while($get_arr=mysql_fetch_assoc($sel_conf)){
	...
	$row.="<td align='center'><a href='edit_glb_brg_err.php?errorid=".$get_arr['pk_bridge_id']."$redto' onclick='return funcalert()' title='Edit Bridge details'>Edit</a></td></tr>";
	$q++;
	$tcond.=" and pk_bridge_id!='".$get_arr['pk_bridge_id']."'";

}
$finalot1=$head.$row."</table>";
}

if($brg_cnt!=mysql_num_rows($sel_conf) || $brg_cnt==0)
	{	
		$sel_brg_sql=mysql_query("select * from tbl_bridge_raw where conference_pk_id=$id".$tcond);
		while($disp_arr=mysql_fetch_assoc($sel_brg_sql)){
			if($q%2==0)
				$rowclr='#EEF2FD';
			else $rowclr='#f6f6f6';
			 $body.="<tr style='background:$rowclr'>";
			 foreach($disp_arr as $key=>$ech_cnt){
				
				 if($key!='pk_bridge_id' && $key!='created_date' && $key!='modified_date' && $key!='master_id' && $key!='conference_pk_id')
					$body.="<td align='center'>".$ech_cnt."</td>";
			 }
			$body.="<td align='center'>RAW</td><td align='center'>-</td><td align='center'>-</td></tr>";
			$q++;
		}
