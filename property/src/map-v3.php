...
<script type="text/javascript">
	...
	var resCountr=-1,Totresult, resultInterval, favCounter=0;
		...
		<?php }
			...
			{	
				...
				$res_place['website']=str_replace("'","",stripslashes($res_place['website']));
				 ...
				 {
					$resultInfoString = "<table width='200' align='center' border='0' cellpadding='5' cellspacing='0'><tr><td align='left'><b>".$res_place['name']."</b></td></tr><tr><td align='left'>".$res_place['address']."</td></tr><tr><td align='left'><a href='".$res_place['website']."'>".$res_place['website']."</a></td></tr><tr><td align='left' id='tdShowDistance_".$countPlaces."' style='display:none'>Distance:</td></tr> <tr><td align='left' id='tdShowDuration_".$countPlaces."' style='display:none'>Duration:</td></tr> <tr style='display:rut;'><td align='left'>TravelMode: <select id='drpdwn_mode_".$countPlaces."'><option value='DRIVING'>Driving</option><option value='WALKING'>Walking</option><option value='BICYCLING'>Bicycling</option><option value='TRANSIT'>Transit</option></select></td></tr><tr><td align='left'><input type='button' id='btnShowRoute_".$countPlaces."' value='Show Route' onclick=\\\"calcRoute(this,".$res_place['lat'].",".$res_place['lan'].",\\'".$res_place['name']."\\')\\\"></td></tr></table>";
				 }
				 else
				 {
	  ...
	  function searchRequest(){
		...
		function callback(results, status) {
		  ...
		  if (status == google.maps.places.PlacesServiceStatus.OK) {			
			...
			Totresult=results;
			resultInterval = setInterval(function (){new addSearchMarker(Totresult)},  100);
	...
	function showInfoWindow(instance,referObj){
		if(lastOpenedInfoWindow != "")
			  lastOpenedInfoWindow.close();

		if(searchInfoWindows[instance].getContent()!="")
		{
			searchInfoWindows[instance].open(map,searchMarkers[instance]);			
		}
		else
		{
			setPlaceDetailsOnclick(referObj);
		}
	...
	function addSearchMarker(results) {
		...
		{				
			...
			setPlaceDetails(place.reference);
			...
			google.maps.event.addListener(marker, 'click', function(event) {
				...
				if(lastOpenedInfoWindow != "")
				   ...
				   {
					setPlaceDetailsOnclick(place.reference);
				   }
				  else
				  {					
		...
		searchResultDisplayString += "<tr><td align='left' style='cursor:pointer;' onclick='showInfoWindow("+resCountr+",\""+place.reference+"\");'><img src='"+markerIcon+"' border='0' /></td><td align='left' onclick='showInfoWindow("+resCountr+",\""+place.reference+"\");' style='cursor:pointer;color:blue;font-size:15px;'>"+place.name+"</td></tr>";

		if((results.length - 1) == resCountr){
			showResult();
 ...
 function getFavInfoWindowInstance(reference){
	...
	function setPlaceDetailsOnclick(referenceParam){
		...
		geocoderService.getDetails(request, function(result, state) {	
		  if (state == google.maps.places.PlacesServiceStatus.OK) {
				...
				if(result.rating)
				 ...
				 if(website!="")
				 {	
						resultInfoString = "<table width='200' align='center' border='0' cellpadding='5' cellspacing='0'><tr><td align='left'><b>"+name+"</b></td></tr><tr><td align='left'>"+result.formatted_address+"</td></tr><tr><td align='left'><a href='"+website+"'>"+website+"</a></td></tr><tr><td align='left'><input type='button' id=\"btnAddFavPlc_"+countPlaces+"\" value='Add to Favorites' onclick=\"initAddPlaceFav('"+strname+"','"+formatted_address+"','"+website+"','"+types+"','"+id+"','"+reference+"','"+rating+"','"+result.geometry.location.lng()+"','"+result.geometry.location.lat()+"','"+countPlaces+"')\"><input type='button' id=\"btnShowRoute_"+countPlaces+"\" value='Show Route' onclick=\"calcRoute(this,"+result.geometry.location.lat()+","+result.geometry.location.lng()+",'"+name+"')\" style=\"display:none\"></td></tr></table>";
				 }
				 else
				 {					
					resultInfoString = "<table width='200' align='center' border='0' cellpadding='5' cellspacing='0'><tr><td align='left'><b>"+name+"</b></td></tr><tr><td align='left'>"+result.formatted_address+"</td></tr><tr><td align='left'><input type='button' id=\"btnAddFavPlc_"+countPlaces+"\" value='Add to Favorites' onclick=\"initAddPlaceFav('"+strname+"','"+formatted_address+"','"+website+"','"+types+"','"+id+"','"+reference+"','"+rating+"','"+result.geometry.location.lng()+"','"+result.geometry.location.lat()+"','"+countPlaces+"')\"><input type='button' id=\"btnShowRoute_"+countPlaces+"\" value='Show Route' onclick=\"calcRoute(this,"+result.geometry.location.lat()+","+result.geometry.location.lng()+",'"+name+"')\" style=\"display:none\"></td></tr></table>";
				 }

				var infoWindowInstance = getInfoWindowInstance(name);
				infoWindowInstance.setContent(resultInfoString);
				infoWindowInstance.open(map,getMarkerInstance(name));
			}			
		});
}
...
function calcRoute(btObj,endLat,endLng,plcname){
    ...
    if(selectedHome!="")
	{ 
		var routTypeId=btObj.id.replace("btnShowRoute_",'drpdwn_mode_');		
		var routType="DRIVING";
		if(document.getElementById(routTypeId))			
		 routType=document.getElementById(routTypeId).value;
		...
		if(btObj.value == "Show Route"){
			  ...
			  if (status == google.maps.DirectionsStatus.OK) 
			   {
				  directionsDisplay.setDirections(response);
				   ...
				   var plcDistance=0,plcDuration=0;
				   for(var rut=0;rut<arrRoutes.length;rut++)
				   {
					 var arrLegs=arrRoutes[rut].legs;
					  for(var lg=0;lg<arrLegs.length;lg++)
					  {
						...
						plcDuration+=parseInt(arrLegs[lg].duration.value);
					  }
				   }
				   ...
				   if(newInfoContent.indexOf("&nbsp;&nbsp;")>0 && newInfoContent.indexOf("Duration:")>0)
				   {
					  var curDuration=getInterString(newInfoContent,"Duration:","&nbsp;&nbsp;");
					  newInfoContent = newInfoContent.replace(curDuration,'');						  
				   }

				   newInfoContent = newInfoContent.replace("Distance:","Distance: "+plcDistance.toFixed(2)+" Miles");
				   newInfoContent = newInfoContent.replace("Duration:","Duration: "+rectime(plcDuration)+"&nbsp;&nbsp;");
				   newInfoContent = newInfoContent.replace(/display:none/g,"display:");
				
				    /*if(document.getElementById(routTypeId))		
					{
					  //document.getElementById(routTypeId).value=routType;
					  //document.getElementById(routTypeId).style.display="none";
					}*/					
			...
			newInfoContent = newInfoContent.replace(/display:/g,"display:none");			
			var arrPopupcnt1 = newInfoContent.split('Distance:');
			var arrPopupcnt2 = newInfoContent.split('Miles');		    
			newInfoContent = arrPopupcnt1[0]+"Distance: "+arrPopupcnt2[1];	
			
			/*
			arrPopupcnt1 = newInfoContent.split('Duration:');
			arrPopupcnt2 = newInfoContent.split('&nbsp;&nbsp;');		    
			newInfoContent = arrPopupcnt1[0]+"Duration:"+arrPopupcnt2[1];
			*/
			...
			{			
			  //document.getElementById(routTypeId).value=routType;
			  //document.getElementById(routTypeId).style.display="";
			}
			*/
  ...
  function getInterString(str,startStr,endStr)
  {
 	strIndx= str.indexOf(startStr)+startStr.length;
	strLength= str.indexOf(endStr)-strIndx;	
	strRes=str.substr(strIndx,strLength);
 	return strRes;
  }
