...
<script type="text/javascript">
	...
	var resCountr=-1,Totresult;
	var isInfoWindowOpened = false;

	var directionDisplay;
	var directionsService = new google.maps.DirectionsService();
	var selectddPlc="",selectedHome="";
	var countPlaces=0;
	...
	<?php
		...
		<?php }
			...
			$sql_branch=mysql_query("select name,iconid from fav_place group by name");			
	...
	function addSearchMarker(results) {
		...
		google.maps.event.addListener(marker, 'click', function(event) {
			if(lastOpenedInfoWindow != "")
				...
				selectddPlc = new google.maps.LatLng(event.latLng.lat(), event.latLng.lng()); //using for Route
		});
	...
	function setPlaceDetails(referenceParam){
		...
		geocoderService.getDetails(request, function(result, state) {		
		  if (state == google.maps.places.PlacesServiceStatus.OK) {
				...
				//alert(name+"','"+formatted_address+"','"+website+"','"+types+"','"+id+"','"+reference+"','"+rating+"','"+result.geometry.location.lng()+"','"+result.geometry.location.lat());
				 
				 countPlaces++;			
				 if(website!="")
				 {
					 if(isFavPlace(name) != -1)
						resultInfoString = "<table width='200' align='center' border='0' cellpadding='5' cellspacing='0'><tr><td align='left'><b>"+name+"</b></td></tr><tr><td align='left'>"+result.formatted_address+"</td></tr><tr><td align='left'><a href='"+website+"'>"+website+"</a></td></tr><tr><td align='left'><input type='button' id=\"btnShowRoute_"+countPlaces+"\" value='Show Route' onclick=\"calcRoute("+result.geometry.location.lat()+","+result.geometry.location.lng()+")\" style=\"display:none\"></td></tr></table>";
					 else
						resultInfoString = "<table width='200' align='center' border='0' cellpadding='5' cellspacing='0'><tr><td align='left'><b>"+name+"</b></td></tr><tr><td align='left'>"+result.formatted_address+"</td></tr><tr><td align='left'><a href='"+website+"'>"+website+"</a></td></tr><tr><td align='left'><input type='button' id=\"btnAddFavPlc_"+countPlaces+"\" value='Add to Favorites' onclick=\"initAddPlaceFav('"+strname+"','"+formatted_address+"','"+website+"','"+types+"','"+id+"','"+reference+"','"+rating+"','"+result.geometry.location.lng()+"','"+result.geometry.location.lat()+"','"+countPlaces+"')\"><input type='button' id=\"btnShowRoute_"+countPlaces+"\" value='Show Route' onclick=\"calcRoute("+result.geometry.location.lat()+","+result.geometry.location.lng()+")\" ></td></tr></table>";
				 }
				 else
				 {
					 if(isFavPlace(name) != -1)
						resultInfoString = "<table width='200' align='center' border='0' cellpadding='5' cellspacing='0'><tr><td align='left'><b>"+name+"</b></td></tr><tr><td align='left'>"+result.formatted_address+"</td></tr><tr><td align='left'><input type='button' id=\"btnShowRoute_"+countPlaces+"\" value='Show Route' onclick=\"calcRoute("+result.geometry.location.lat()+","+result.geometry.location.lng()+")\" ></td></tr></table>";
					else
						resultInfoString = "<table width='200' align='center' border='0' cellpadding='5' cellspacing='0'><tr><td align='left'><b>"+name+"</b></td></tr><tr><td align='left'>"+result.formatted_address+"</td></tr><tr><td align='left'><input type='button' id=\"btnAddFavPlc_"+countPlaces+"\" value='Add to Favorites' onclick=\"initAddPlaceFav('"+strname+"','"+formatted_address+"','"+website+"','"+types+"','"+id+"','"+reference+"','"+rating+"','"+result.geometry.location.lng()+"','"+result.geometry.location.lat()+"','"+countPlaces+"')\"><input type='button' id=\"btnShowRoute_"+countPlaces+"\" value='Show Route' onclick=\"calcRoute("+result.geometry.location.lat()+","+result.geometry.location.lng()+")\" style=\"display:none\"></td></tr></table>";
				 }
...
function clearRoute(){
    ...
    try
	{
	  ...
	  document.getElementById('td-directions-panel').style.display="none";
	}
	catch(err)
	{
		//alert(err.message);
...
function calcRoute(endLat, endLng){
	...
	{ 
		...
		var directionsRendererOptions={suppressMarkers:true};
